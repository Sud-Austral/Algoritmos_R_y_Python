...
```{r cinco}
 ...
 covid19 <-
  read_xlsx("C:/Users/usuario/Desktop/GitHub-trabajo/Algoritmos_R_y_Python/R0/R0_R/insumo.xlsx")
...
for(t in ux)
{
    ...
    if (t != "No Informado")
    {
        ...
        if (nrow(comuna_en_cuestion) != 0 && (comuna_en_cuestion$"Poblacion" != 0))
        {
            ...
            infectados.por.dia <<- cumsum(as.numeric(comuna_en_cuestion$"Casos Diarios"))

            fallecidos.por.dia <<- cumsum(as.numeric(comuna_en_cuestion$"Fallecidos Diarios"))

            infectados.por.dia2 = as.numeric(infectados.por.dia + fallecidos.por.dia)
            
            recuperados.por.dia = cumsum(as.numeric(comuna_en_cuestion$"Recuperados Diarios"))
...
print(tabla.comuna)
 ...
 # comuna_en_cuestion$"Población"                  )
        ...
        eee <-
        data.frame(comuna_v, R0_v , fecha_v)
